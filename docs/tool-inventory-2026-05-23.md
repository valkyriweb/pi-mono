...
| **#1C** | ⏸ Queued — attempted, reverted | Delete the 6 Uppercase factories (`createUppercaseReadTool` etc.) and the `toolName?: "x" \| "X"` option from `read.ts`/`edit.ts`/`write.ts`/`grep.ts`/`find.ts`/`ls.ts`, plus the 6 Uppercase switch arms / `allToolNames` entries / `ToolName` type entries from `core/tools/index.ts`. The `native-tool-aliases` extension already provides the runtime substitutes. **Attempted 2026-05-23 and reverted clean.** Mechanical edits worked + `npm run check` was green + most affected tests passed via a shared `test/native-tool-aliases-factory.ts` helper that mirrors the my-pi extension. Real blocker: `test/suite/extensions/fork-agent.test.ts > intersects allowedTools with the parent's active tool list` failed because the fork's `effectiveTools` came back `[]` instead of `["Read"]`. `harness.session.getActiveToolNames()` did contain `Read` (the extension factory loaded fine), but the in-prompt `forkAgent` flow either (a) snapshots `parentActiveTools` from a path that bypasses extension-provided tools, or (b) hits the `resolveEffectiveTools` non-fork branch (when `parentSystemPrompt` isn't set on the first turn) where `agent.tools` allowlist filters out extension tools. Needs deeper trace + likely a small tweak in `agents/executor.ts` or `_forkAgentFromExtension` to honor extension tools the same way `getActiveToolNames` does. Branch was deleted; tree is clean. Estimated net **-450 LOC** from core once the fork-agent invariant is sorted. |
