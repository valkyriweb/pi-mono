...
| **#1C** | ⏸ Queued | Delete the 6 Uppercase factories (`createUppercaseReadTool` etc.) and the `toolName?: "x" \| "X"` option from `read.ts`/`edit.ts`/`write.ts`/`grep.ts`/`find.ts`/`ls.ts`, plus the 6 Uppercase switch arms / `allToolNames` entries / `ToolName` type entries from `core/tools/index.ts`. The `native-tool-aliases` extension already provides the runtime substitutes. Wider test surface than #1B (`tools.test.ts:36` enumerates all Uppercase names, `tools.test.ts:870` instantiates `createUppercaseGrepToolDefinition` directly). Same migration approach as #1B — inline factory in tests. Estimated net **-450 LOC** from core. |
