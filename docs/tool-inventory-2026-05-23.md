...
| **#1B** | ⏸ Blocked on test migration | Delete the now-redundant Uppercase factories + `toolName` option + `web-fetch.ts` + `web-search.ts` from `pi-mono-fork/packages/coding-agent/src/core/tools/`. Net ~500 LOC removed from core. **Real blocker found 2026-05-23**: `packages/coding-agent/test/claude-bridge-native-tools.test.ts` and `tools.test.ts` create sessions WITHOUT extensions and assert WebFetch/WebSearch/Read/Edit/etc. are present. Need either (a) test-harness extension-load helper, or (b) migrate these tests to `my-pi/extensions/native-tool-aliases/test-*.ts` running against a full pi session. Earlier stale-ctx bug concern was a red herring. |
