...
| **#1B** | ⏸ Awaiting decision | Delete the now-redundant Uppercase factories + `toolName` option + `web-fetch.ts` + `web-search.ts` from `pi-mono-fork/packages/coding-agent/src/core/tools/`. Net ~500 LOC removed from core. Blocked on triage of pre-existing stale-ctx loader bug in `pi-otel-exporter`/`session-title`. |
