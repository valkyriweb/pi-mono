...
| **#1C** | ⏸ Queued — deeper invariant than expected | Delete the 6 Uppercase factories (`createUppercaseReadTool` etc.) and the `toolName?: "x" \| "X"` option from `read.ts`/`edit.ts`/`write.ts`/`grep.ts`/`find.ts`/`ls.ts`, plus the 6 Uppercase switch arms / `allToolNames` entries / `ToolName` type entries from `core/tools/index.ts`. The `native-tool-aliases` extension already provides the runtime substitutes. **Attempted twice 2026-05-23 and reverted clean.** Mechanical edits worked + `npm run check` was green + most affected tests passed via a shared `test/native-tool-aliases-factory.ts` helper that mirrors the my-pi extension. Real blocker: `test/suite/extensions/fork-agent.test.ts > intersects allowedTools with the parent's active tool list` failed with the fork's `effectiveTools` coming back `[]` instead of `["Read"]`. Confirmed via test assertion that `harness.session.getActiveToolNames()` DOES contain `Read` at the parent level. Then added an unconditional `throw` at the entry of `_forkAgentFromExtension` and it never fired during the failing test — meaning `ctx.forkAgent()` is going through a different code path entirely from what `runner.forkAgentFn = _forkAgentFromExtension` claims. The bypass path needs to be located before extraction can complete cleanly. Tree clean; branches deleted. Estimated net **-450 LOC** from core once the bypass is found. |
