...
run_probe "Native agent tool schema/modes" grep -nE 'agentToolSchema|controlActionSchema|background|runId|executeAgentControlAction|taskSchema|tasks|chain|context|agentScope|exactly one mode|createAgentToolDefinition' "$repo/packages/coding-agent/src/core/tools/agent.ts"
...
run_probe "Native task lifecycle action probe and generic control disambiguation" grep -nE 'taskId|action|Type\.Literal\("create"|Type\.Literal\("list"|Type\.Literal\("get"|Type\.Literal\("update"|deleted|activeForm|blockedBy|metadata' "$repo/packages/coding-agent/src/core/tools/agent.ts"
cat >> "$source_out" <<'EOF'
...
Native now exposes `background: true` plus generic background-run status/detail/interrupt/cancel/resume control through the `agent` tool and `/agents-status` hints. This is source-backed and covered by the cheap status capture, but full background-control runtime behavior was not stress-tested with a paid child run.
EOF
...
Current native `agent` schema supports single/parallel/chain plus generic background-run control. Generic `action`/`runId` status/interrupt/cancel/resume control is not the requested task-record lifecycle: `taskId`, `activeForm`, dependencies/metadata/status updates, and create/list/get/update/delete task lifecycle actions are not present.
EOF
