...
mode="${1:?usage: run-tmux-scenario.sh <native|subagents> <capture-name> <prompt-or-command>}"
name="${2:?usage: run-tmux-scenario.sh <native|subagents> <capture-name> <prompt-or-command>}"
prompt="${3:?usage: run-tmux-scenario.sh <native|subagents> <capture-name> <prompt-or-command>}"
...
launch_args() {
  case "$mode" in
    ...
    subagents)
      ...
      ;;
    *)
      echo "usage: run-tmux-scenario.sh <native|subagents> <capture-name> <prompt-or-command>" >&2
      exit 2
      ;;
  esac
}
...
sleep "${PI_AGENT_EVAL_SCENARIO_WAIT:-5}"
{
