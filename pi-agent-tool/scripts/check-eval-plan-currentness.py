...
def write_markdown(path: Path, metrics: dict[str, int]) -> None:
    lines = [
        ...
        f"| token caveat | {metrics['eval_plan_token_caveat']} | Rubric names native S01/S05 paid probes including interrupt/resume and cancel, prior extension fallthroughs, and current extension no-child caveat. |",
        f"| native S05 live background | {metrics['eval_plan_s05_native_background_live']} | S05 native plan names the paid background start/status, interrupt/resume, and cancel probes. |",
        f"| prior extension tmux caveat | {metrics['eval_plan_prior_extension_tmx_caveat']} | S05-S07 extension rows mark prior tmux evidence as prior/current-runtime-blocked. |",
        f"| summary refs current | {metrics['eval_plan_summary_refs_current']} | Evidence manifest, findings, runbook, and autoresearch summaries name the expanded current/prior eval-plan scope. |",
...
def main() -> int:
    ...
    s05 = get_scenario_row(plan, "S05")
    s06 = get_scenario_row(plan, "S06")
    s07 = get_scenario_row(plan, "S07")
    prior_extension_text = "\n".join(" | ".join(row) for row in [s05, s06, s07])
    ...
    metrics = {
        ...
        "eval_plan_token_caveat": int(
            ...
            and "native s05 paid background start/status, interrupt/resume, and cancel probes" in token_section.lower()
        ...
        "eval_plan_s05_native_background_live": int(
            "paid live background start/status probe" in " | ".join(s05).lower()
            and "paid live interrupt/resume probe" in " | ".join(s05).lower()
            and "paid live cancel probe" in " | ".join(s05).lower()
        ),
        "eval_plan_prior_extension_tmx_caveat": int(
            prior_extension_text.lower().count("prior") >= 3
            and prior_extension_text.lower().count("current extension load is blocked") >= 3
            and prior_extension_text.lower().count("current runtime caveat") >= 3
        ),
        "eval_plan_summary_refs_current": int(
            "native S05 paid start/status, interrupt/resume, and cancel probes" in manifest
            and "S05-S07 extension tmux/fallthrough captures as prior evidence" in manifest
            and "S05-S07 prior extension captures with current-runtime caveats" in manifest
            and "S05-S07 prior extension tmux/fallthrough caveats" in findings
            and "S05-S07 prior-extension/current-runtime ambiguity" in runbook
            and "current live/failure/prior evidence" in autoresearch
        ),
