...
NATIVE_CONTROL_ACTIONS = ["status", "detail", "interrupt", "cancel", "resume"]
NATIVE_CONTROL_FIELDS = ["action", "runId", "background"]
...
def native_state() -> dict[str, object]:
    ...
    control_action_hits = {action: type_literal_present(source, action) for action in NATIVE_CONTROL_ACTIONS}
    control_field_hits = {field: word_present(source, field) for field in NATIVE_CONTROL_FIELDS}
    ...
    if "action" in lifecycle_field_hits and lifecycle_actions_present == 0:
        # A generic agent control discriminator (status/interrupt/cancel/resume)
        # is not the requested task-record lifecycle action surface.
        lifecycle_field_hits["action"] = False
    ...
    control_actions_present = sum(control_action_hits.values())
    control_fields_present = sum(control_field_hits.values())
    ...
    return {
        ...
        "control_action_hits": control_action_hits,
        "control_field_hits": control_field_hits,
        ...
        "control_actions_present": control_actions_present,
        "control_fields_present": control_fields_present,
        "control_not_task_lifecycle": int(control_actions_present > 0 and lifecycle_actions_present == 0 and lifecycle_fields_present == 0),
...
def write_markdown(path: Path, native: dict[str, object], extension: dict[str, object]) -> None:
    ...
    control_action_hits: dict[str, bool] = native["control_action_hits"]  # type: ignore[assignment]
    control_field_hits: dict[str, bool] = native["control_field_hits"]  # type: ignore[assignment]
    ...
    lines.extend(
        [
            ...
            f"- Generic native control fields present: {native['control_fields_present']}.",
            f"- Generic native control actions present: {native['control_actions_present']}.",
            f"- Generic native control is task lifecycle: {int(not native['control_not_task_lifecycle'])}.",
            "",
            "## Native generic control guard",
            "",
            "Generic background-run control can improve S05, but it is not the S09 task-record lifecycle surface unless lifecycle task actions and fields are present too.",
            "",
            "| Generic native control marker | Present |",
            "|---|---:|",
        ]
    )
    for field in NATIVE_CONTROL_FIELDS:
        lines.append(f"| field `{field}` | {str(control_field_hits[field]).lower()} |")
    for action in NATIVE_CONTROL_ACTIONS:
        lines.append(f"| action `{action}` | {str(control_action_hits[action]).lower()} |")
    lines.extend(
        [
            "",
    ...
    lines.extend(
        [
            ...
            "- If the native lifecycle fields/actions/statuses land, this audit should fail and S09 must be rescored instead of preserving the pending verdict.",
            "- Generic background-run control is tracked separately because it affects S05 but is not task-record lifecycle by itself.",
            "",
        ]
    )
    path.write_text("\n".join(lines))
...
def main() -> int:
    ...
    print(f"task_lifecycle_native_control_fields_present={native['control_fields_present']}")
    print(f"task_lifecycle_native_control_actions_present={native['control_actions_present']}")
    print(f"task_lifecycle_native_control_not_task_lifecycle={native['control_not_task_lifecycle']}")
