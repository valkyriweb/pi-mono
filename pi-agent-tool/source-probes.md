...
Generated: 2026-05-03T18:39:25Z
...
239:Child tools are computed from the parent active tools, requested tools, agent allow/deny lists, and a global recursive `agent` denial. `--tools`, `--no-builtin-tools`, and `--no-tools` continue to set the parent ceiling; children cannot gain tools the parent does not have active.
245:| `--tools <list>`, `-t <list>` | Allowlist specific built-in, extension, and custom tools, including `agent` |
246:| `--no-builtin-tools`, `-nbt` | Disable built-in tools but keep extension/custom tools enabled |
258:| `--no-extensions` | Disable extension discovery |
270:pi --no-extensions -e ./my-extension.ts
# exit=0
```
...
22:	{ name: "agents-status", description: "Show native child-agent runs and background controls" },
# exit=0
```
...
596:				rawKeyHint("!", "to run bash"),
597:				rawKeyHint("!!", "to run bash (no context)"),
2452:			if (text === "/agents-doctor") {
2457:			if (text === "/agents-status" || text.startsWith("/agents-status ")) {
2459:				const detailId = text.startsWith("/agents-status ") ? text.slice(15).trim() : undefined;
2466:				await this.handleAgentsCommand(agentArgs);
3907:	private async handleAgentsCommand(args?: string): Promise<void> {
3940:		if (args === "list-chains") {
3949:		if (args.startsWith("run ")) {
3951:			await this.session.prompt(`Use the native agent tool to run ${agentId} with this task: ${task}`);
3954:		if (args.startsWith("parallel ")) {
3961:				`Use the native agent tool parallel mode for these agents: ${agents.join(", ")}. Task for each: ${task}`,
3965:		if (args.startsWith("run-chain ")) {
3982:				`Use the native agent tool chain mode to run saved chain ${chain.name}${task ? ` for this task: ${task}` : ""}. Chain input: ${chainInput}`,
# exit=0
```
...
```bash
$ grep -nE agentToolSchema\|controlActionSchema\|background\|runId\|executeAgentControlAction\|taskSchema\|tasks\|chain\|context\|agentScope\|exactly\ one\ mode\|createAgentToolDefinition /Users/luke/Projects/personal/pi-mono-fork/packages/coding-agent/src/core/tools/agent.ts
17:const contextModeSchema = Type.Union([
34:const controlActionSchema = Type.Union([
42:const taskSchema = Type.Object({
46:	context: Type.Optional(contextModeSchema),
47:	extraContext: Type.Optional(Type.String({ description: "Additional task-specific context" })),
55:export const agentToolSchema = Type.Object({
56:	action: Type.Optional(controlActionSchema),
57:	runId: Type.Optional(Type.String({ description: "Background run id for control actions" })),
62:	tasks: Type.Optional(Type.Array(taskSchema, { maxItems: 8 })),
63:	chain: Type.Optional(Type.Array(taskSchema, { minItems: 1 })),
65:	context: Type.Optional(contextModeSchema),
72:	chainDir: Type.Optional(Type.String({ description: "Base directory for relative chain outputs" })),
73:	background: Type.Optional(
74:		Type.Boolean({ description: "Run in the background and return immediately with a run id" }),
76:	agentScope: Type.Optional(Type.Union([Type.Literal("user"), Type.Literal("project"), Type.Literal("both")])),
79:export type AgentToolInput = Static<typeof agentToolSchema>;
92:		Boolean(params.tasks && params.tasks.length > 0),
93:		Boolean(params.chain && params.chain.length > 0),
99:	tasks: NonNullable<AgentToolInput["tasks"]>;
102:	const hasParallel = Boolean(params.tasks && params.tasks.length > 0);
105:		throw new Error("agent tool requires exactly one mode: {agent, task}, {tasks}, or {chain}");
110:			tasks: [
115:					context: params.context,
126:	if (hasParallel) return { mode: "parallel", tasks: params.tasks ?? [] };
127:	return { mode: "chain", tasks: params.chain ?? [] };
221:function runControlHint(runId?: string): string | undefined {
222:	return runId
223:		? `Control: /agents-status ${runId}, /agents interrupt ${runId}, /agents cancel ${runId}, /agents resume ${runId} [-- prompt]`
240:	if (details.background && details.status === "running") {
242:			`agent ${details.mode}: background running${details.runId ? ` · ${details.runId}` : ""}`,
244:			runControlHint(details.runId),
277:		runId: result.run.id,
278:		background: result.run.execution === "background",
284:async function executeAgentControlAction(params: AgentToolInput): Promise<AgentToolResult<AgentToolDetails>> {
286:		throw new Error("agent tool control actions cannot be combined with {agent, task}, {tasks}, or {chain}");
291:		return { content: [{ type: "text", text: formatAgentStatus(undefined, params.runId) }] };
293:	if (!params.runId) throw new Error(`agent control action ${action} requires runId`);
296:			? await interruptAgentRecentRun(params.runId)
298:				? await cancelAgentRecentRun(params.runId)
299:				: await resumeAgentRecentRun(params.runId, params.message);
300:	const detailText = formatAgentStatus(undefined, params.runId);
309:	ctx: Parameters<ToolDefinition<typeof agentToolSchema>["execute"]>[4],
311:	const scope = params.agentScope;
325:export function createAgentToolDefinition(
328:): ToolDefinition<typeof agentToolSchema, AgentToolDetails> {
333:			"Launch a built-in or configured Pi child agent. Supports single {agent, task}, parallel {tasks}, sequential chain {chain}, background execution, and background run control actions.",
336:			"Use agent for delegated work that benefits from an isolated child context.",
337:			"When parallel exploration or review is needed, send multiple agent tool-use blocks in one assistant message; Pi runs those calls concurrently. Use tasks[] only for explicit batched fan-out inside one agent call.",
338:			"Use background:true for long-running delegated work that should continue while the parent reports back; control it with action/status/interrupt/cancel/resume and runId.",
341:		parameters: agentToolSchema,
344:			if (params.action) return executeAgentControlAction(params);
353:					tasks: mode.tasks,
355:					context: params.context,
362:					chainDir: params.chainDir,
363:					background: params.background,
364:					agentScope: params.agentScope,
380:		renderCall(args, theme, context) {
381:			const text = (context.lastComponent as Text | undefined) ?? new Text("", 0, 0);
385:					label = `${args.action}${args.runId ? `: ${args.runId}` : ""}`;
388:					const names = mode.tasks.map((task) => task.agent).join(", ");
389:					label = `${mode.mode}${args.background ? " background" : ""}: ${names}`;
397:		renderResult(result, options, _theme, context) {
398:			const component = (context.lastComponent as Container | undefined) ?? new Container();
422:export function createAgentTool(cwd: string, options?: AgentToolOptions): AgentTool<typeof agentToolSchema> {
423:	return wrapToolDefinition(createAgentToolDefinition(cwd, options));
# exit=0
```
...
23:	sessionRefs: Array<{ agent: string; sessionId?: string; sessionPath?: string }>;
67:		outputPaths: [...run.outputPaths],
68:		sessionRefs: run.sessionRefs.map((session) => ({ ...session })),
79:	return runs.map((run) => run.outputPath).filter((path): path is string => Boolean(path));
82:function summarizeSessions(runs: AgentRunDetails[]): AgentRecentRun["sessionRefs"] {
83:	return runs.map((run) => ({ agent: run.agent, sessionId: run.sessionId, sessionPath: run.sessionPath }));
91:	return run.execution === "background" && run.status === "interrupted" && run.sessionRefs.length === 1;
95:	run.outputPaths = summarizeOutputs(runs);
96:	run.sessionRefs = summarizeSessions(runs);
155:		outputPaths: [],
156:		sessionRefs: [],
249:	if (!run.usage) return undefined;
251:		run.usage.cacheRead || run.usage.cacheWrite ? ` cache r/w ${run.usage.cacheRead}/${run.usage.cacheWrite}` : "";
252:	const cost = run.usage.cost.total > 0 ? ` $${run.usage.cost.total.toFixed(4)}` : "";
253:	return `${run.usage.totalTokens} tok${cache}${cost}`;
258:	lines.push(`   session: ${run.sessionPath ?? run.sessionId ?? "n/a"}`);
271:	const usage = formatUsage(run);
272:	if (usage) lines.push(`   usage: ${usage}`);
273:	if (run.outputPath) lines.push(`   output: ${run.outputPath}`);
282:export function formatAgentStatus(runs = listAgentRecentRuns(), detailId?: string): string {
284:		"Native agent status",
286:		"Background control: native background runs support status, interrupt, cancel, and single-run resume.",
306:		const outputs = run.outputPaths.length > 0 ? ` outputs: ${run.outputPaths.join(", ")}` : "";
307:		const sessions =
308:			run.sessionRefs.length > 0
309:				? ` sessions: ${run.sessionRefs
310:						.map((s) => s.sessionId ?? s.sessionPath)
317:			`${run.id} ${run.mode} ${run.execution} ${run.status}${resumable} ${formatDuration(run)} agents: ${run.agents.join(", ")}${sessions}${outputs}${error}`,
# exit=0
```
...
## Native task lifecycle action probe and generic control disambiguation
...
56:	action: Type.Optional(controlActionSchema),
57:	runId: Type.Optional(Type.String({ description: "Background run id for control actions" })),
58:	message: Type.Optional(Type.String({ description: "Optional resume/continue prompt for control actions" })),
286:		throw new Error("agent tool control actions cannot be combined with {agent, task}, {tasks}, or {chain}");
288:	const action = params.action;
289:	if (!action) throw new Error("Missing agent control action");
290:	if (action === "status" || action === "detail") {
293:	if (!params.runId) throw new Error(`agent control action ${action} requires runId`);
295:		action === "interrupt"
297:			: action === "cancel"
333:			"Launch a built-in or configured Pi child agent. Supports single {agent, task}, parallel {tasks}, sequential chain {chain}, background execution, and background run control actions.",
338:			"Use background:true for long-running delegated work that should continue while the parent reports back; control it with action/status/interrupt/cancel/resume and runId.",
344:			if (params.action) return executeAgentControlAction(params);
...
Note: `action` hits in this probe are generic background-run control (`status`/`detail`/`interrupt`/`cancel`/`resume`) unless task-record lifecycle fields/actions such as `taskId`, `activeForm`, `metadata`, or `Type.Literal("create"|"list"|"get"|"update")` are also present. See `task-lifecycle-audit.md`, `native-control-currentness.md`, `native-control-tests.md`, `native-background-control-live.md`, `native-background-interrupt-resume-live.md`, and `native-background-cancel-live.md` for the split, source/unit-test control evidence, and paid start/status, interrupt/resume, and cancel probes.
