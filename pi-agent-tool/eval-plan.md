...
| S05 | Async/background/status/control | `/agents-status` plus source-backed background status/detail/interrupt/cancel/resume control, unit tests, one paid live background start/status probe, one paid live interrupt/resume probe, and one paid live cancel probe | `async`, status/interrupt/resume tool actions; `/subagents-status` removed; prior fallthrough tmux capture only because current extension load is blocked until loader fix/rerun | source + paid native start/status/interrupt-resume/cancel + prior extension tmux + current runtime caveat |
| S06 | Doctor/diagnostics | `/agents-doctor` | `/subagents-doctor` exists in source with a prior loaded-extension capture; current extension load is blocked until loader fix/rerun | source + prior tmux + current runtime caveat |
| S07 | UI manager/selector pass | `/agents` selector/scaffold | requested `/subagents` manager unavailable in 0.24.0; prior removed-command probe shows fallthrough cost; current extension load is blocked until loader fix/rerun | source + prior fallthrough tmux + current runtime caveat |
