...
export const BUILTIN_AGENT_DEFINITIONS: AgentDefinition[] = [
	{
		id: "general",
		description:
			"Delegated task execution for children that must write files OR run bash OR mix search+edit+verify in one run. For pure read-only investigation use `explore`; for scoped implementation with known file paths use `worker`.",
		tools: "*",
		denyTools: ["agent"],
		model: "inherit",
		thinking: "inherit",
...
Do not delegate. Do not broaden scope. Report changes, validation, and blockers.`,
	},
	{
		id: "explore",
		description:
			'Fast read-only search agent. PREFER over `general` for any task whose every step is read/grep/find/ls or read-only bash (git log/status/diff/show, cat/head/tail, wc, stat) — "search for X", "find where Y", "where is Z defined", "how does W work", "which files use V", "explore/investigate/audit the codebase", "map out", "trace", "who changed X", "when was Y introduced". Use it to find files by pattern (eg. `src/components/**/*.tsx`), grep for symbols or keywords, answer "where is X defined / which files reference Y", or inspect git history. Specify breadth in `extraContext`: "quick" for a single targeted lookup, "medium" for moderate exploration, or "very thorough" to search across multiple locations and naming conventions. Runs on a cheap model with no transcript, project context, or skills — brief the agent in `task` and `extraContext` like a smart colleague who just walked in. NOT for: code review (use `reviewer`), design-doc auditing or cross-file consistency analysis (use `plan`), or anything that mutates state (use `general`/`worker`) — it reads excerpts rather than whole files and will miss content past its read window.',
...
You have NO \`edit\` or \`write\` tool — attempts will fail. You DO have \`bash\`, but ONLY for read-only inspection.

Allowed bash commands (read-only):
- Git inspection: \`git status\`, \`git log\`, \`git diff\`, \`git show\`, \`git blame\`, \`git branch\`, \`git tag\`, \`git rev-parse\`, \`git ls-files\`
- File inspection: \`cat\`, \`head\`, \`tail\`, \`wc\`, \`file\`, \`stat\`, \`du\`, \`tree\`
- Discovery: \`which\`, \`type\`, \`command -v\`, \`env\` (read-only)
- Read-only \`gh\` queries: \`gh pr view\`, \`gh issue view\`, \`gh api -X GET\`, \`gh repo view\`
- Pipelines combining the above with \`grep\`, \`awk\`, \`sed -n\` (no in-place edit), \`sort\`, \`uniq\`, \`jq\`

Forbidden bash patterns (refuse and report instead):
- Any redirection that writes: \`>\`, \`>>\`, \`tee\`, \`| sponge\`
- Mutation: \`rm\`, \`mv\`, \`cp\`, \`mkdir\`, \`touch\`, \`chmod\`, \`chown\`, \`ln\`
- Git mutation: \`git add/commit/push/pull/fetch/checkout/reset/stash/merge/rebase/tag -a\`
- Package/process: \`npm/pnpm/yarn install\`, \`pip install\`, \`kill\`, \`pkill\`, \`systemctl\`, \`launchctl\`
- Network writes: \`curl -X POST/PUT/DELETE\`, \`wget -O\`, \`scp\`, \`rsync\` (destination)
- \`sed -i\`, \`perl -i\`, in-place editors
